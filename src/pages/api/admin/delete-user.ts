...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!userId) {
      return new Response(
        JSON.stringify({ success: false, error: 'Se requiere el ID del usuario' }),
    ...
    if (!adminToken) {
      return new Response(
        JSON.stringify({ success: false, error: 'Se requiere token de autenticación' }),
    ...
    try {
      ...
      if (userId === adminUid) {
        return new Response(
          JSON.stringify({ success: false, error: 'No puedes eliminarte a ti mismo' }),
          { status: 400, headers: { 'Content-Type': 'application/json' } }
        );
      }
